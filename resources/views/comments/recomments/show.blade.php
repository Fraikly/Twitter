<div class="comments" style="margin-bottom: 33px;margin-left: 8%;">
    <div style="margin: 0;margin-bottom: 15px;">
        @if(\Illuminate\Support\Facades\Auth::user()->id===$reComment->user_id)
            <div class="comment_edit">
                ...
                <form method="post" action="{{route('comments.delete',[$twit->id, $reComment->id])}}"
        ...
        @endif
        <a href="{{route('users.show',$reComment->user_id)}}"> <img
                src="{{asset('/storage/' .\App\Models\User::find($reComment->user_id)->picture)}}"
    ...
    right: 32%;">
            ...
            @if( !\Illuminate\Support\Facades\Auth::check() or \App\Models\User::find(\Illuminate\Support\Facades\Auth::user()->id)->likesForComment()->where('comment_id',$reComment->id)->get()->isEmpty())
                <form method="post" action="{{route('comment.likes.store',[$twit->id, $reComment->id])}}"
                      style="display: inline-block;    margin-bottom: 0;" id="create_likes">
                    @csrf
                    <input type="image" src="{{URL::to('/')}}\img\for_interface\no_like_twit.png" style="width: 24px"
                           alt="Submit" class="likes_button">
                </form>

            @else
                <form method="post" action="{{route('comment.likes.delete',[$twit->id, $reComment->id])}}"
                      style="display: inline-block;    margin-bottom: 0;" id="delete_likes">
                    @csrf
                    @method('delete')
                    <input type="image" src="{{URL::to('/')}}\img\for_interface\like_twit.png" style="width: 24px"
                           alt="Submit" class="likes_button">
                </form>
            @endif
