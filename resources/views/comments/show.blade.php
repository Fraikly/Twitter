<div class="comment">
    ...
    right: 32%;">
            ...
            @if( !\Illuminate\Support\Facades\Auth::check() or \App\Models\User::find(\Illuminate\Support\Facades\Auth::user()->id)->likesForComment()->where('comment_id',$comment->id)->get()->isEmpty())
                <form method="post" action="{{route('comment.likes.store',[$twit->id, $comment->id])}}"
            ...
            @else
                <form method="post" action="{{route('comment.likes.delete',[$twit->id, $comment->id])}}"
            ...
            <form method="get" action="{{route('recomments.create',[$twit->id, $comment->id])}}"
                  style="display: inline-block;    margin-bottom: 0;" id="delete_likes">
                @csrf
                <input type="image" src="{{URL::to('/')}}\img\for_interface\answer.png" style="width: 24px" alt="Submit"
                       class="likes_button">
            </form>
        </div>
    </div>
