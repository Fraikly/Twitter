        ...
        <div class="twit_content">
            ...
            <div class="likes">
                ...
                @if( !\Illuminate\Support\Facades\Auth::check() or \App\Models\User::find(\Illuminate\Support\Facades\Auth::user()->id)->likesForTwit()->where('twit_id',\App\Models\Twit::find($twit->original_twit)->id)->get()->isEmpty())
                    <form method="post"
                          action="{{route('likes.store',\App\Models\Twit::find($twit->original_twit)->id)}}"
                ...
                @else
                    <form method="post"
                          action="{{route('likes.delete',\App\Models\Twit::find($twit->original_twit)->id)}}"
                ...
                <a href="{{route('comments.index',\App\Models\Twit::find($twit->original_twit)->id)}}">
                    <input type="image" src="{{URL::to('/')}}\img\for_interface\comments.png" style="width: 28px"
                           alt="Submit"
                           class="likes_button">
                    <label
                        class="likes_count">   {{\App\Models\Twit::find($twit->original_twit)->comments()->count()}}</label></a>

                <a href="{{route('retwits.create',\App\Models\Twit::find($twit->original_twit)->id)}}">
                    <input type="image" src="{{URL::to('/')}}\img\for_interface\retwit.png" style="width: 28px"
                           alt="Submit"
                           class="likes_button">
                    <label
                        class="likes_count"> {{\App\Models\Twit::where('original_twit',\App\Models\Twit::find($twit->original_twit)->id)->count()}}</label></a>
            </div>
